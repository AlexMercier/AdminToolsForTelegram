...
public class AdapterKickedUsers extends BaseAdapter {
    ...
    public View getView(int position, View view, ViewGroup parent) {
        ...
        final ImageView ivIsMuted = UIUtils.getHolderView(view, R.id.ivIsMuted);
        ...
        ivIsMuted.setVisibility(banUser.isMuted ? View.VISIBLE : View.INVISIBLE);

        if (banUser.isMuted) {
            tvBanText.setText(R.string.text_user_has_muted_ban);
        } else if (banUser.banText != null)
        ...
        if (user.status == null) {
            tvUserInfo.setText("");
        } else if (user.status.getConstructor() == TdApi.UserStatusRecently.CONSTRUCTOR) {
            ...
            String time = CommonUtils.tsToDate(offline.wasOnline);
            ...
            tvUserInfo.setText(R.string.status_online);
        } else {
            tvUserInfo.setText("");
        }
        ...
        if (banUser.banAge == 0)
            ...
            s += mCtx.getString(R.string.text_ban_temporary) + " " + banUser.getBanExpiredTime();// "Временный бан. Дата разбана: "+banUser.getBanExpiredTime();
        }
        if (s.isEmpty()) {
